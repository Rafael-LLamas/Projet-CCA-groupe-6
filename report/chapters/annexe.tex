...
\paragraph{int gr\_mat\_reconstruct\_A\_v2(gr\_mat\_t A, gr\_mat\_t G, gr\_mat\_t H, disp\_type\_t type, gr\_ctx\_t ctx);}
Is the function that reconstructs the original matrix $A$ from its generator matrices $G$ and $H$.
...
Function that we implement to apply the downshift matrix $Z$ to a matrix $M$ and store the result in $Res$.
\paragraph{int gr\_mat\_apply\_Zt(gr\_mat\_t Res, gr\_mat\_t M, gr\_ctx\_t ctx);}
Function that we implement to apply the transpose of the downshift matrix $Z^T$ to a matrix $M$ and store the result in $Res$.


\subsubsection*{\underline{Flint function}}\label{annexe:FLINT}
gr\_mat\_neg
gr\_mat\_concat\_horizontal
