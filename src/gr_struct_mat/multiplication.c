...
int _gr_mat_mul_vector(gr_mat_t Res, gr_mat_t G, gr_mat_t H, gr_mat_t V, gr_ctx_t ctx)
{
...
int _gr_mat_mul_generator(gr_mat_t G_c, gr_mat_t H_c, gr_mat_t T, gr_mat_t U, gr_mat_t G, gr_mat_t H, gr_ctx_t ctx)
{
  ...
  status |= _gr_mat_mul_vector(W, T, U, Tmp, ctx);
  status |= _gr_mat_apply_Z(W, W, ctx);


  // 2. V = B^T * U (Le générateur de B^T est {H, G})
  status |= _gr_mat_mul_vector(V, H, G, U, ctx);
  ...
  status |= _gr_mat_mul_vector(a, T, U, LastCol_m, ctx);
  ...
  status |= _gr_mat_mul_vector(b, H, G, LastCol_k, ctx);
