...
int main(void)
{
  ...
  for (slong i = 0; i < 4; i++)
  {
    for (slong j = 0; j < 4; j++)
    {
      ...
      status |= gr_set_si(gr_mat_entry_ptr(testing_mat_source, i, j, ctx), toeplitz_val, ctx);
    }
  }
  printf("\n\nSource Matrix:\n");
  ...
  status |= gr_struct_mat_init_set(testing_mat, testing_mat_source, T_TOEPLITZ, DISP_PLUS, ctx);
  printf("\n");
  gr_struct_mat_print(testing_mat, ctx);

  // Get the dense matrix from the structured matrix type
  gr_mat_t reconstructed_dense_mat;
  gr_mat_init(reconstructed_dense_mat, 0, 0, ctx);
  status |= gr_struct_mat_reconstruct(reconstructed_dense_mat, testing_mat, ctx);
