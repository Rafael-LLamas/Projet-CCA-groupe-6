...
int test_random_quasi_toeplitz() {
  ...
  while (i < 50) {
    ...
    gr_mat_t ran, D;
    gr_ctx_init_nmod(ctx, n_randprime(state, 64, 1));
    slong rank = n_randint(state, 9) + 1;
...
int test_random_generator_quasi_toeplitz() {
  int error = GR_SUCCESS;
  int i = 0;
  flint_rand_t state;
  flint_rand_init(state);
  ...
  while (i < 50) {
    ...
    gr_ctx_t ctx;
    gr_ctx_init_nmod(ctx, n_randprime(state, 64, 1));
    slong rank = n_randint(state, 19) + 1;
    slong n = n_randint(state, 100) + rank;
    slong m = n_randint(state, 100) + rank;
    gr_mat_init(G, n, rank, ctx);
    gr_mat_init(H, m, rank, ctx);
    gr_mat_init(A, n, m, ctx);
    gr_mat_init(D, n, m, ctx);
...
int main(int argc, char *argv[]) {
  ...
  } else if (strcmp("gr_mat_random_generator_quasi_toeplitz", argv[1]) == 0) {
    ok = test_random_generator_quasi_toeplitz();
  } else {
